...
REM 妫€鏌?Git 鐘舵€?echo 馃搵 妫€鏌?Git 鐘舵€?..
git status

REM 娣诲姞鎵€鏈夋洿鏀?echo 馃搧 娣诲姞鎵€鏈夋枃浠?..
git add .
...
if %errorlevel% == 0 (
    echo 鉁?杩滅▼浠撳簱宸插瓨鍦?) else (
    echo 鉂?璇峰厛璁剧疆杩滅▼浠撳簱锛?    echo git remote add origin https://github.com/YOUR_USERNAME/focus-forest.git
    pause
    exit /b 1
)
...
echo 5. 璁剧疆鐜鍙橀噺锛?echo    VITE_SUPABASE_URL=https://zutwxkfdvzgglbzntoux.supabase.co
...
echo 6. 鐐瑰嚮 'Deploy'
echo.
echo 馃寪 閮ㄧ讲瀹屾垚鍚庯紝鎮ㄥ皢鑾峰緱锛?echo    - 鍏紑璁块棶 URL
echo    - 鑷姩 HTTPS
echo    - 鍏ㄧ悆 CDN
echo.
echo 馃帀 閮ㄧ讲瀹屾垚锛佹偍鐨?Focus Forest 搴旂敤鍗冲皢涓婄嚎锛?pause
